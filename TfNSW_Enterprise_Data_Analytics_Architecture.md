...
- Cross-domain KPI inconsistency
...
```text
DATA PRODUCTS -> GOLD -> SEMANTIC MODEL -> CERTIFIED KPIs
                                      |
             +------------------------+-----------------------+
             |                        |                       |
...
3. Calculate variance against the approved baseline.
...
6. Generate facts first, interpretation second, with evidence.
7. Apply user permissions to every retrieved source.
8. Log the interaction and evidence under applicable policy.
...
```text
Executive KPI: On-time Performance
        |
Semantic measure
        |
Service Performance data product
        |
...
WEATHER / EVENTS ----------------+
                 ...
                 POWER BI                     AI
                    |                           |
              Trends/corridor          "Why was it late?"
                    |                           |
                    +-------------+-------------+
                                  |
                         Governed evidence
...
3. Calculate delay/reliability using certified definitions.
...
8. Retain evidence and source references.
...
Business outcome → capability → data domain → source → ingestion → integration → data product → semantic metric → consumption → governance/security → operating model.
